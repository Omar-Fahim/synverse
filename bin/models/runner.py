...
class Runner(ABC):
    ...
    def train_model_given_config(self, config, best_n_epochs, validation = False, save_output=True):
        ...
        if not validation: #train model with both training and validation data
            ...
            train_loader = DataLoader(self.triplets_scores_dataset, batch_size=self.batch_size, shuffle=True, num_workers=10, pin_memory=True)
            ...
            for fold in range(self.n_folds):
                ...
                train_loader = DataLoader(train_subsampler, batch_size=self.batch_size, shuffle=True, num_workers=10, pin_memory=True)
                val_loader = DataLoader(val_subsampler, batch_size=self.batch_size, shuffle=False, num_workers=10, pin_memory=True)
    ...
    def get_test_score(self, test_df, best_model_state, config, save_output=True, file_prefix = '_'):
        ...
        :return:
        '''
        test_loader = DataLoader(self.get_triplets_score_dataset(test_df, score_name=self.score_name), batch_size=4096, shuffle=True, num_workers=10, pin_memory=True)
