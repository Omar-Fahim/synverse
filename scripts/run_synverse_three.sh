#!/bin/bash -l
#SBATCH --job-name=synverse_a100_regular_three
...
envsubst < assets/testdata/Cluster_Config_three.yaml > $RESOLVED_CFG
...
nextflow run . \
  -profile conda,gpu \
  --input $RESOLVED_CFG \
  -work-dir $TMPDIR/regular_work \
  --outdir $WORK/synverse/regular_results_three \
